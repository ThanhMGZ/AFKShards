...
public class ItemData {
    ...
    public static ItemStack getItemStack(String data) {
        data = data.replace("%n%","\n").replace("%r%","\\\r").replace("%s%","\\").replace("%p%","+");
        ItemStack item = null;
        try {
            ItemStack[] stack1 = itemStackArrayFromBase64(data);
    ...
    public static String getData(ItemStack stack) {
        ...
        return itemStackArrayToBase64(inventory.getContents()).replace("\n","%n%").replace("\\\r","%r%").replace("\\","%s%").replace("\\+","%p%");
    }
}
